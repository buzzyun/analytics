...
public class WeeklyTypeSearchLogAnalyticsTask extends AnalyticsTask<TypeSearchLog> {
	...
	protected void prepare(Calendar calendar) {
		...
		//1주일치의 월별 로그를 데이터베이스를 사용하여 머징한다.
		...
		// calc를 카테고리별로 모두 만든다.
		Calculator<TypeSearchLog> yearlyTypeHitCalculator = new WeeklyTypeHitCalculator("Yearly type hit calculator", calendar, baseDir, siteId, categoryIdList, typeList);
