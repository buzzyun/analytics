...
public class GetRelateKeyWordListAction extends AuthAction {
	...
	public void doAuthAction(ActionRequest request, ActionResponse response) throws Exception {
		...
		AnalyticsDBService keywordService = ServiceManager.getInstance().getService(AnalyticsDBService.class);

		MapperSession<RelateKeywordMapper> mapperSession = keywordService.getMapperSession(RelateKeywordMapper.class);

		try {
			RelateKeywordMapper relateKeywordMapper = mapperSession.getMapper();
			...
			if (relateKeywordMapper != null) {
				...
				if (totalSize > 0) {
					...
					for (RelateKeywordVO vo : list) {
						...
						resultWriter.object().key("ID").value(vo.getId()).key("KEYWORD").value(vo.getKeyword()).key("VALUE").value(vo.getValue()).endObject();
					}
				}
			}
