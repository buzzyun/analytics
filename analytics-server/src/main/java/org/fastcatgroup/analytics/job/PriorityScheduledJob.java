...
/**
 * 입력된 여러 스케쥴은 한번에 하나씩만 수행된다. 스케쥴 시간이 겹치는 경우, priority가 낮은 job이 우선하고 모두 실행되는 것이 보장된다. 
 * */
public class PriorityScheduledJob extends ScheduledJob {
	...
	public PriorityScheduledJob(String key, ScheduledJobEntry scheduledJobEntry) {
		super(key);
		this.priorityJobQueue = new PriorityQueue<ScheduledJobEntry>(5, comparator);
	...
	public JobResult doRun() throws AnalyticsException {
		...
		while (!isCanceled) {
			try {
				...
				logger.info("Next {} job will run {} at {} after waiting {}ms", key(), actualJob.getClass().getSimpleName(), entry.getStartTime(), timeToWait);
				if(timeToWait > 0){
					synchronized (this) {
						wait(timeToWait);
					}
