...
#-------------------------------------------------------------------------------
#Fastcat-analytics start script
...
start_daemon() {
	...
	trap '' 1 2
	"$java_path" -Dserver.home=$server_home $JVM_OPTS $JAVA_OPTS $ADDITIONAL_OPTS -classpath $LIB/analytics-server-bootstrap.jar org.fastcatsearch.analytics.server.Bootstrap start > $OUTPUT_LOG 2>&1 &
	PID=`echo "$!"`
	sleep 1
	...
	then
		...
		echo "$java_path -Dserver.home=$server_home $JVM_OPTS $JAVA_OPTS $ADDITIONAL_OPTS -classpath $LIB/analytics-server-bootstrap.jar org.fastcatsearch.analytics.server.Bootstrap start > $OUTPUT_LOG 2>&1 &"
		echo "################################"
...
stop_daemon() {
	if [ -f ".pid" ] ; then
		...
		ps -ef|grep org.fastcatsearch.analytics.server.Bootstrap|grep -v grep
	fi
	return 1
}
...
if [ "$1" = "run" ] ; then
	"$java_path" -Dserver.home=$server_home $JVM_OPTS $JAVA_OPTS -classpath $LIB/analytics-server-bootstrap.jar org.fastcatsearch.analytics.server.Bootstrap start
